...
Например, в [Events API](EventsAPIv1.md#одно-событие) или в [Games API](GamesAPIv1.md#одна-игра).

## Быстрые ссылки

* [Пример одного события](#пример-одного-события)
* [Пример одной игры](#пример-одной-игры)
* [Описание свойств](#описание-свойств)
* [Запросы на ваш эндпоинт](#запросы-на-ваш-эндпоинт)

## Пример одного события

Подробнее см. [здесь](EventsAPIv1.md#одно-событие)
...
Подробнее см. [здесь](GamesAPIv1.md#одна-игра)
...
|                          |             |              | в разделе [Даты и время](EventsAPIv1.md#даты-и-время) |
...
|                          |             |              | [Повторная отправка той же регистрации](#повторная-отправка-той-же-регистрации) |
...
[ниже](#повторная-отправка-той-же-регистрации). Всё необходимое для проверки, что запрос от нас,
...
`registration.teams[].id` [мероприятия](EventsAPIv1.md#одно-событие). Без него ни одна из сторон не сможет
...
## Повторная отправка той же регистрации
