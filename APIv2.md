...
```php
md5($questId . $datetime . $secret)
```

Используемые параметры:

* questId - ID квеста, для которого получаются/отправляются данные (по умолчанию такой же, как в URL, например у "Похищения" = 12, http://extrareality.by/quest/12), но можно использовать и внутренний ID с вашего сайта, тогда сообщите нам об этом
